...
// ML Coordinator Design Doc: go/shodan-ml-doc
...
impl MLCoordinator {
    ...
    pub fn periodic(&mut self, id: ImageId, rate_in_ms: u32) -> Result<(), MlCoordError> {
        ...
        kata_timer_periodic(idx as TimerId, rate_in_ms).map_err(|_| MlCoordError::InvalidTimer)
    }
    ...
    pub fn cancel(&mut self, id: &ImageId) -> Result<(), MlCoordError> {
        ...
        if self.models[model_idx]
            ...
            .is_some()
        {
            kata_timer_cancel(model_idx as TimerId).map_err(|_| MlCoordError::InvalidTimer)?;
        }

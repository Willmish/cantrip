...
fn capscan_command(
    ...
    match args.next() {
        ...
        Some("memory") => {
            let _ = kata_memory_interface::kata_memory_capscan();
        }
        Some("process") => {
...
fn install_command(
    ...
    let cnode = kata_cnode_alloc(cnode_depth).map_err(|_| CommandError::Memory)?; // XXX leaks pkg_contents
    pkg_contents

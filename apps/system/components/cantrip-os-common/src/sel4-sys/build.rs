...
fn main() {
    ...
    let sel4_dir = env::var("SEL4_DIR")
        .unwrap_or_else(|_| format!("{}/cantrip/kernel", env::var("ROOTDIR").unwrap()));
    ...
    println!("PLATFORM {:?}", env::var("PLATFORM"));
    let sel4_out_dir = env::var("SEL4_OUT_DIR").unwrap_or_else(|_| {
        format!(
            "{}/out/cantrip/{}/release/kernel",
            env::var("ROOTDIR").unwrap(),
            env::var("PLATFORM").unwrap(),
        )
    });
    ...
    for feature in features {
        ...
        if feature.as_str() == "CONFIG_KERNEL_MCS" {
            has_mcs = true;
        }
    }
    ...
    cmd.arg(&python_bin).arg("tools/syscall_stub_gen.py");
    if has_mcs {
        cmd.arg("--mcs");
    }
    cmd.args(&[
